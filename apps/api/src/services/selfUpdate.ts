...
export function buildSelfUpdateScript(
  ...
  return [
    ...
    "  [ -f .env ] || return 1",
    "  database_env_tmp=\"$(mktemp .env.composebastion-database.XXXXXX)\" || return 1",
    "  {",
    "    cat .env",
    "    printf '\\n# ComposeBastion managed legacy database transition\\n'",
    "    printf 'DATABASE_URL=\\n'",
    "  } > \"$database_env_tmp\" || { rm -f -- \"$database_env_tmp\"; return 1; }",
    "  chmod 600 \"$database_env_tmp\" || { rm -f -- \"$database_env_tmp\"; return 1; }",
    "  mv -- \"$database_env_tmp\" .env",
    "}",
    "container_id() {",
    ...
    "    if stack_is_ready \"$wait_expected_app_version\" \"$wait_expected_worker_version\" \"$wait_expected_app_image_id\" \"$wait_expected_worker_image_id\" \"${5:-ready}\"; then return 0; fi",
    ...
    "  if ! write_outcome failed working_directory not_required 1; then printf '%s\\n' 'Self-update could not publish the working_directory outcome.' >&2; fi",
    "  release_lock",
    "  exit 1",
    "fi",
    "owner_wait=0",
    ...
    "  if ! write_outcome failed lock_ownership not_required 75; then printf '%s\\n' 'Self-update could not publish the lock_ownership outcome.' >&2; fi",
    "  exit 75",
    "fi",
    "gate_wait=0",
    ...
    "if ! env -u DATABASE_URL \"$DOCKER_BIN\" compose --env-file .env -f \"$SOURCE_ENV_PROBE_TEMPLATE_PATH\" config --format json > \"$SOURCE_ENV_PROBE_PATH\"; then fail_update compose_config 1; fi",
    ...
    "if ! preparation_output=\"$(run_upgrade_preparation reconcile)\"; then fail_update prepare 1; fi",
    "printf '%s\\n' \"$preparation_output\"",
    ...
    "  canonicalize) if ! canonicalize_managed_database_environment; then fail_update env_canonicalization 1; fi ;;",
    "  preserve) ;;",
    ...
    "if [ \"$success_cleanup_failed\" -eq 1 ]; then printf '%s\\n' \"Self-update succeeded, but protected recovery cleanup was incomplete at $UPGRADE_STATE_DIR.\" >&2; fi",
    "release_lock",
    "exit 0"
  ].join("\n");
}
...
export function buildSelfUpdateLockLaunchScript(input: {
  ...
  return [
    ...
    "set -C",
    `if ! exec 3> ${shQuote(logPath)}; then set +C; release_launcher_lock; exit 1; fi`,
    "set +C",
    `nohup ${shQuote(scriptPath)} >&3 2>&1 < /dev/null & child_pid=$!`,
    "exec 3>&-",
