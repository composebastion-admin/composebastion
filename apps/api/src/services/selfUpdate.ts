...
export function buildSelfUpdateScript(
  ...
  return [
    ...
    "      printf '  app:\\n    image: %s\\n    pull_policy: never\\n' \"$PREVIOUS_APP_IMAGE_ID\"",
    "      printf '  worker:\\n    image: %s\\n    pull_policy: never\\n' \"$PREVIOUS_WORKER_IMAGE_ID\"",
    "    } > \"$ROLLBACK_COMPOSE_PATH\"",
    "  fi",
    "  if [ \"$rollback_ready\" -eq 1 ] && \"$DOCKER_BIN\" compose -f \"$COMPOSE_FILE\" -f \"$ROLLBACK_COMPOSE_PATH\" up -d --pull never --force-recreate app worker >/dev/null 2>&1 && wait_for_stack \"$PREVIOUS_APP_VERSION\" \"$PREVIOUS_WORKER_VERSION\" \"$PREVIOUS_APP_IMAGE_ID\" \"$PREVIOUS_WORKER_IMAGE_ID\" && [ \"$environment_restored\" -eq 1 ] && [ \"$references_restored\" -eq 1 ]; then",
    "    rollback_status=succeeded",
    "  fi",
    "  rm -f -- \"$ENV_BACKUP_PATH\" \"$GATE_PATH\" \"$ROLLBACK_COMPOSE_PATH\"",
    ...
    "if [ -f .env ]; then",
    "  if ! cp -p -- .env \"$ENV_BACKUP_PATH\"; then fail_update env_backup 1; fi",
    "  ENV_EXISTED=1",
    "else",
    "  : > \"$ENV_BACKUP_PATH\"",
    "fi",
    ...
    "if ! \"$DOCKER_BIN\" compose -f \"$COMPOSE_FILE\" up -d --force-recreate app worker; then fail_update up 1; fi",
    "if ! wait_for_stack \"$TARGET_VERSION\" \"$TARGET_VERSION\" '' ''; then fail_update verification 1; fi",
