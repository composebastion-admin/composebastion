...
const componentSchemas = {
  ...
  GithubRepository: object(["id", "name", "repositoryUrl", "owner", "repo", "branch", "composePath", "projectName", "env", "defaultHostId", "hostCloneUrl", "hostCloneDirectory", "lastDeployedAt", "lastDeployedCommitSha", "latestCommitSha", "updateCheckedAt", "updateCheckError", "hasGithubToken", "githubTokenStatus", "githubTokenCheckedAt", "githubTokenCheckError", "lastError", "createdAt", "updatedAt"], {
    ...
    updateCheckedAt: stringOrNullSchema,
    updateCheckError: stringOrNullSchema,
    hasGithubToken: { type: "boolean" },
    githubTokenStatus: enumSchema(["none", "unchecked", "valid", "error"]),
    githubTokenCheckedAt: stringOrNullSchema,
    githubTokenCheckError: stringOrNullSchema,
    lastError: stringOrNullSchema,
    createdAt: dateTimeSchema,
    updatedAt: dateTimeSchema
  }),
  GithubRepositoriesResponse: namedArrayResponse("repositories", schemaRef("GithubRepository")),
  GithubRepositoryResponse: namedItemResponse("repository", schemaRef("GithubRepository")),
  DeploymentSource: object(["id", "sourceType", "name", "sourceLocator", "branch", "composePath", "workingDir", "projectName", "defaultHostId", "targetHostIds", "safeEnvironment", "hasCredential", "metadata", "lastDeployedAt", "createdAt", "updatedAt"], {
    id: idSchema,
    ...
    projectName: { type: "string" },
    defaultHostId: idOrNullSchema,
    targetHostIds: arrayOf(idSchema),
    safeEnvironment: recordSchema,
  ...
  DeploymentSourceResponse: namedItemResponse("source", schemaRef("DeploymentSource")),
  DeploymentSourcesResponse: namedArrayResponse("sources", schemaRef("DeploymentSource")),
  DeploymentAnalysis: object(["id", "hostId", "sourceId", "sourceType", "sourceInput", "status", "env", "summary", "variables", "warnings", "blockers", "registryIssues", "expiresAt", "createdAt", "updatedAt"], {
    id: idSchema,
    hostId: idSchema,
    ...
    composePath: stringOrNullSchema,
    workingDir: stringOrNullSchema,
    composeYaml: stringOrNullSchema,
    env: { type: "string" },
    summary: recordSchema,
    variables: arrayOf(recordSchema),
    warnings: arrayOf(recordSchema),
    blockers: arrayOf(recordSchema),
    registryIssues: arrayOf(recordSchema),
    error: stringOrNullSchema,
    expiresAt: dateTimeSchema,
    createdAt: dateTimeSchema,
    updatedAt: dateTimeSchema,
    deployedAt: stringOrNullSchema
  }),
  DeploymentAnalysisResponse: namedItemResponse("analysis", schemaRef("DeploymentAnalysis")),
  DeploymentAnalysisJobResponse: object(["analysis", "job"], {
    analysis: schemaRef("DeploymentAnalysis"),
    job: schemaRef("OperationJob")
  }),
  ...
  DeploymentSourceUpdateRequest: object([], {
    name: { type: "string" },
    branch: stringOrNullSchema,
    composePath: stringOrNullSchema,
    workingDir: stringOrNullSchema,
    ...
    defaultHostId: idOrNullSchema,
    safeEnvironment: recordSchema,
    credentialUsername: stringOrNullSchema,
  ...
  GithubBranchesResponse: namedArrayResponse("branches", { type: "string" }),
  GithubAccessCheck: object(["ok", "checkedAt", "repositoryPrivate", "error"], {
    ok: { type: "boolean" },
    checkedAt: dateTimeSchema,
    repositoryPrivate: { type: ["boolean", "null"] },
    error: stringOrNullSchema
  }),
  GithubAccessCheckResponse: namedItemResponse("access", schemaRef("GithubAccessCheck")),
  GithubStoredAccessCheckResponse: object(["repository", "access"], {
    repository: schemaRef("GithubRepository"),
    access: schemaRef("GithubAccessCheck")
  }),
  GithubComposePreviewResponse: object(["repository", "branch", "composeYaml", "projectName", "env"], {
    repository: schemaRef("GithubRepository"),
    branch: { type: "string" },
    composeYaml: { type: "string" },
    projectName: { type: "string" },
...
export function buildOpenApiDocument() {
  ...
  return {
    ...
    info: {
      ...
      description: "ComposeBastion API contract. Stable JSON endpoints are available under /api/v1 while legacy /api routes remain compatible in V1."
    },
    servers: [{ url: "/" }],
    components: {
      securitySchemes: {
...
export function buildOpenApiMarkdown() {
  const lines = [
    ...
    "",
    "Stable JSON endpoints are documented under `/api/v1/*`. Existing `/api/*` endpoints remain compatibility aliases in V1.",
    "",
