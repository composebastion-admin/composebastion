...
- Keep app and agent images on the same release when possible. The most recent
  published bridge release is `v1.1.3`.
...
The only qualified 1.2 route is `1.0.6/1.1.2 -> 1.1.3 -> 1.2`. Reach this
...
credential plus the exact 1.1.3 images with `--no-deps` if verification fails.
