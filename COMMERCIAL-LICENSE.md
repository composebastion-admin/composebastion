# Commercial Licensing

ComposeBastion is source-available software, not open-source software.

Free use is allowed only for home labs, home use, private use, personal
learning, and private non-commercial testing.

Prior written approval or a purchased license is required before using
ComposeBastion in any business, company, employer, client, customer, MSP, SaaS,
hosted, government, school, nonprofit, production, non-production, testing,
staging, proof-of-concept, evaluation, or organizational environment.

## Commercial Use Examples

Approval or a paid license is required for:

- company or employer use;
- internal business testing;
- business proof-of-concept work;
- development, staging, or production environments;
- consultant, agency, MSP, or client work;
- customer, tenant, or third-party environment management;
- hosted, SaaS, cloud, or managed-service use;
- resale, redistribution, sublicensing, repackaging, or bundling;
- paid support, training, implementation, or consulting offerings;
- competing products or services;
- any use that gives direct or indirect value to a business or organization.

## Request Approval

To request approval, written permission, or a commercial license, contact:

support@composebastion.com

Do not include secrets, credentials, customer data, pricing expectations, or
confidential commercial terms in public issues.

Commercial terms are valid only when provided in a separate written agreement or
written approval from the project publisher.
